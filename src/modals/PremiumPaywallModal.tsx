...
export function PremiumPaywallModal({
  ...
  trigger,
  onClose,
}: PremiumPaywallModalProps): React.JSX.Element | null {
  const {t} = useTranslation();
  const insets = useSafeAreaInsets();

  const storeVisible = useAppStore(s => s.isPaywallVisible);
  const storeTrigger = useAppStore(s => s.paywallTrigger);
  const closePaywall = useAppStore(s => s.closePaywall);
  const purchasePlan = useAppStore(s => s.purchasePlan);
  const restorePurchases = useAppStore(s => s.restorePurchases);
  const showToast = useAppStore(s => s.showToast);

  const isVisible = visible ?? storeVisible;
  const activeTrigger = trigger ?? storeTrigger;

  if (!isVisible) return null;

  const triggerMap: Record<PaywallTrigger, string> = {
  ...
  return (
    <View style={styles.overlay} testID="premium-paywall-modal">
      ...
      <View style={[styles.sheet, {paddingBottom: insets.bottom + spacing.xl}]}>
        <ScrollView
          ...
          <Pressable
            testID="premium-paywall-close-button"
            onPress={close}
          ...
          <Pressable
            testID="premium-paywall-buy-lifetime"
          ...
          <Pressable
            testID="premium-paywall-buy-monthly"
          ...
          <Pressable
            testID="premium-paywall-restore-button"
